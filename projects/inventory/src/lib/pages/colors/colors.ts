...
export class ColorsPage implements OnInit {
  ...
  editingItem: ColorDto | null = null;

  /** Flashes the newly-created row green and floats it to the top. */
  highlighter = new RowHighlighter();
  private justCreated: ColorDto | null = null;

  formCode = ''; formName = ''; formHexCode = '';
  formColorFamily = ''; formDisplayOrder = 0; formIsActive = true;
  ...
  constructor(private service: ColorService, private cdr: ChangeDetectorRef) {}
  ...
  load(): void {
    ...
    this.service.getAll({
      ...
      error: () => { this.error = 'Failed to load colors'; this.loading = false; this.cdr.detectChanges(); },
    });
  }

  /** List ordered so the freshly-created row sits on top while it's highlighted. */
  get displayItems(): ColorDto[] {
  ...
  openCreateForm(): void {
    ...
    this.resetForm();
    this.formCode = `CLR-${(this.totalCount + 1).toString().padStart(6, '0')}`;
    this.showForm = true;
  }
  openEditForm(item: ColorDto): void {
    this.editingItem = item; this.formCode = item.code ?? ''; this.formName = item.name ?? '';
    this.formHexCode = item.hexCode ?? ''; this.formColorFamily = item.colorFamily ?? '';
    this.formDisplayOrder = item.displayOrder; this.formIsActive = item.isActive; this.showForm = true;
  }
  resetForm(): void { this.formCode = ''; this.formName = ''; this.formHexCode = ''; this.formColorFamily = ''; this.formDisplayOrder = 0; this.formIsActive = true; }
  ...
  save(): void {
    const obs = this.editingItem
      ? this.service.update(this.editingItem.id, {
          ...
          hexCode: this.formHexCode || null, colorFamily: this.formColorFamily || null,
          displayOrder: this.formDisplayOrder, isActive: this.formIsActive,
        } as UpdateColorDto)
      : this.service.create({
          code: this.formCode || null, name: this.formName || null,
          hexCode: this.formHexCode || null, colorFamily: this.formColorFamily || null,
