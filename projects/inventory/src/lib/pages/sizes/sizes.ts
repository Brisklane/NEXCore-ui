...
export class SizesPage implements OnInit {
  ...
  editingItem: SizeDto | null = null;

  /** Flashes the newly-created row green and floats it to the top. */
  highlighter = new RowHighlighter();
  private justCreated: SizeDto | null = null;

  formCode = ''; formName = ''; formSizeChart = '';
  sizeCharts: LookupItemDto[] = [];
  formSortOrder = 0; formIsActive = true;
  ...
  constructor(private service: SizeService, private lookup: InventoryLookupService, private cdr: ChangeDetectorRef) {}
  ngOnInit(): void {
    this.load();
    this.lookup.getAll().subscribe({ next: (res) => { this.sizeCharts = res.data?.sizeCharts ?? []; this.cdr.detectChanges(); } });
  }
  load(): void {
    ...
    this.service.getAll({
      ...
      error: () => { this.error = 'Failed to load sizes'; this.loading = false; this.cdr.detectChanges(); },
    });
  }

  /** List ordered so the freshly-created row sits on top while it's highlighted. */
  get displayItems(): SizeDto[] {
  ...
  openCreateForm(): void {
    ...
    this.resetForm();
    this.formCode = `SZE-${(this.totalCount + 1).toString().padStart(6, '0')}`;
    this.showForm = true;
  }
  openEditForm(item: SizeDto): void {
    this.editingItem = item; this.formCode = item.code ?? ''; this.formName = item.name ?? '';
    this.formSizeChart = item.sizeChart ?? ''; this.formSortOrder = item.sortOrder;
    this.formIsActive = item.isActive; this.showForm = true;
  }
  resetForm(): void { this.formCode = ''; this.formName = ''; this.formSizeChart = ''; this.formSortOrder = 0; this.formIsActive = true; }
  ...
  save(): void {
    const obs = this.editingItem
      ? this.service.update(this.editingItem.id, {
          ...
          sizeChart: this.formSizeChart || null, sortOrder: this.formSortOrder,
