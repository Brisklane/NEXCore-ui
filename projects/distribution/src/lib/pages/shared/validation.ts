 ...
 *
 * Messages are written as instructions ("Give the route a name"), not accusations ("Name is
 * required"), and they name the actual thing on screen. Someone who has just been stopped from
...
export const requiredWhen = (label: string, when: (all: Record<string, unknown>) => boolean): Rule<unknown> =>
  (v, all) => when(all) && (v === null || v === undefined || String(v).trim() === '')
    ? `${label} is needed.` : null;
...
export const percent = (label: string): Rule<number | null> =>
  v => v !== null && v !== undefined && (Number(v) < 0 || Number(v) > 100)
    ? `${label} is a percentage — it has to be between 0 and 100.` : null;
...
export const phone = (label = 'Phone'): Rule<string> =>
  v => {
    const s = (v ?? '').trim();
    if (!s) return null;
    return /^[+]?[\d\s\-()]{7,20}$/.test(s)
      ? null : `${label} does not look like a phone number.`;
  };

/** Latitude/longitude typed by hand on the onboarding form. */
export const latitude = (label = 'Latitude'): Rule<number | null> =>
  v => v !== null && v !== undefined && (Number(v) < -90 || Number(v) > 90)
    ? `${label} must be between -90 and 90.` : null;

export const longitude = (label = 'Longitude'): Rule<number | null> =>
  v => v !== null && v !== undefined && (Number(v) < -180 || Number(v) > 180)
    ? `${label} must be between -180 and 180.` : null;
...
/** Cross-field: an end date that must not land before its start date. */
export const afterDateField = (other: string, label: string, otherLabel: string): Rule<string> =>
  (v, all) => {
    const end = String(v ?? '').trim();
    const start = String(all[other] ?? '').trim();
    if (!end || !start) return null;
    return new Date(end).getTime() < new Date(start).getTime()
      ? `${label} cannot be before ${otherLabel.toLowerCase()}.` : null;
  };
...
/** A date that cannot be in the future — a count sheet, a collection, a declared sale. */
export const notInFuture = (label: string): Rule<string> =>
  v => {
    if (!v) return null;
    return new Date(v).getTime() > Date.now() + 60_000
      ? `${label} cannot be in the future.` : null;
  };

/** A list that has to contain something — order lines, scheme products, selected outlets. */
export const nonEmptyList = (label: string): Rule<unknown[]> =>
  v => !Array.isArray(v) || v.length === 0 ? `Add at least one ${label}.` : null;
